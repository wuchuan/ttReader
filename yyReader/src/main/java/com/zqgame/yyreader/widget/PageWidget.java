...
import com.zqgame.yyreader.Config.TouchEnum;
...
public class PageWidget extends View {
	...
	float mDegrees;
	/**
	 * mTouchToCornerDis开始的翻动点到当前触摸点的直线距离
	 */
...
//    TouchEnum TouchMode=Config.TouchEnum.REAL;
    PointF mOldTouch = new PointF();
    float startX=0;
    boolean isFristMove=true;
    boolean isDirctionLeft=true;
    float moveX=0;
	...
	public boolean doTouchEvent(MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			mTouch.x = event.getX();
			mTouch.y = event.getY();
			if(Config.TouchEnum.LR==Config.TouchMode){
			mOldTouch.x = mTouch.x;
			mOldTouch.y = mTouch.y;
			isFristMove=true;
			}
		}
	...
	private void drawCurrentPageArea(Canvas canvas, Bitmap bitmap, Path path) {
		mPath0.reset();
		mPath0.moveTo(mBezierStart1.x, mBezierStart1.y);//曲线的起始点
		mPath0.quadTo(mBezierControl1.x, mBezierControl1.y, mBezierEnd1.x,
				mBezierEnd1.y);
		//mBezierControl1.x, mBezierControl1.y(曲线的拉动点)  
		//mBezierEnd1.x mBezierEnd1.y(曲线的终点)
		
		mPath0.lineTo(mTouch.x, mTouch.y);
		mPath0.lineTo(mBezierEnd2.x, mBezierEnd2.y);
		mPath0.quadTo(mBezierControl2.x, mBezierControl2.y, mBezierStart2.x,
				mBezierStart2.y);
		//mBezierControl2.x, mBezierControl2.y(曲线的拉动点)  
		//mBezierStart2.x mBezierStart2.y(曲线的终点)
	 ...
	 * @param canvas
	 * @param bitmap 
	 * 传进来的bitmap（下一页）会被作为背景，
	 * 下一页的上画的倒影，部分会被取剪切，旋转
	 */
	...
	protected void onDraw(Canvas canvas) {
		...
		if(Config.TouchEnum.REAL==Config.TouchMode){
			calcPoints();
		...
		else if(Config.TouchEnum.LR==Config.TouchMode){
			//计算出移动(x)距离,计算出滑动的方向
			//画出背后的页面
			//根据计算画出前面的页面
			//设置当前位置为最后触摸的位置
			//停止滑动的时候，根据方向，自动运行动画覆盖
			
			calcDistanceDirection();
			if(isDirctionLeft){
				drawBackPage(canvas, mNextPageBitmap);
				drawFrontPage(canvas, mCurPageBitmap);
			}
			else{
				drawBackPage(canvas, mCurPageBitmap);
				drawFrontPage(canvas, mNextPageBitmap);
			}
			reLocalPoint();
		}
		else if(Config.TouchEnum.TB==Config.TouchMode){
			
			
		}
	}

	private void reLocalPoint() {
		mOldTouch.x = mTouch.x;
		mOldTouch.y = mTouch.y;
	}

	private void drawBackPage(Canvas canvas, Bitmap BackBitmap) {
		canvas.drawBitmap(BackBitmap, 0, 0, null);
	}

	private void drawFrontPage(Canvas canvas, Bitmap FrontPageBitmap) {
		canvas.drawBitmap(FrontPageBitmap, startX, 0, null);
	}

	private void calcDistanceDirection() {
		moveX=mTouch.x-mOldTouch.x;
		if(isFristMove){
			if(moveX>0){
				startX=-Config.SCREEN_WIDTH;
				isDirctionLeft=false;
			}
			else{
				startX=0;
				isDirctionLeft=true;
...
//	public void setTouchMode(TouchEnum TouchMode){
